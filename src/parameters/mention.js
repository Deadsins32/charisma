...
module.exports = function(input, passthrough) {
    ...
    if (input.startsWith('<@')) {
        ...
        var members = passthrough.guild.members.filter(function(member) { return (member.nickname && member.nickname.toLowerCase().includes(input)) || member.user.username.toLowerCase().includes(input) });
