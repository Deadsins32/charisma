...
module.exports = {
    ...
    command: async function(imports, parameters) {
        ...
        async function parse(name) {
            var status = await command.status({ name: name }, imports);
            if (status) {
        ...
        var configs = imports.commands.configs;
        var list = new Object();
        ...
        if (name) {
            ...
            embed.addField(`usage`, `\`${command.syntax(imports.local.guild.config.prefix, parameters[0], imports)}\``, true);
            ...
            for (var i = 0; i < 10; i++) {
                if (array[(page * 10) + i]) {
                    var syntax = command.syntax(imports.local.guild.config.prefix, array[(page * 10) + i][0], imports);
