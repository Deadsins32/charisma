...
module.exports = {
    ...
    check: function(name, parameters, passthrough) {
        ...
        if (config && config.params.length > 0) {
            ...
            for (let i = 0; i < config.params.length; i++) {
                let error = false;
                if (config.params[i].length != 0) {
                    if (parameters.length == 0 && config.params[i][0].required) { error = true }
                    if (parameters.length > config.params[i].length) { error = true }
                    if (requirements[i] > parameters.length) { error = true }
                    if (parameters.length <= config.params[i].length) {
                ...
                if (parameters.length <= config.params[i].length) {
                    for (let p = 0; p < parameters.length; p++) {
                        if (!passthrough.commands.methods[config.params[i][p].type](parameters[p], passthrough).pass) { error = true }
                    }
                }
            ...
            if (config.params && config.params.length > 0) {
                for (let p = 0; p < config.params.length; p++) {
                    for (let pp = 0; pp < config.params[p].length; pp++) {
                        ...
                        if (config.params[p][pp].required) { syntax += `<${insert}> ` }
                        else { syntax += `[${insert}] ` }
                    }
    
                    if (p != config.params.length - 1) { syntax += `\n${prefix + command} ` }
                }
            }
